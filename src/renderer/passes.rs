...
/// A lightweight vertex used for drawing simple debug quads to the screen.
/// Only has position and a single set of texture coordinates.
#[repr(C)]
...
impl DebugQuadVertex {
    /// Vertex buffer format for `DebugQuadVertex`.
...
const QUAD_VERTS: &[DebugQuadVertex] = &[
    DebugQuadVertex {
